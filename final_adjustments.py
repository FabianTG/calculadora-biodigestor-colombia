#!/usr/bin/env python3
"""
Script para aplicar los ajustes finales solicitados de forma estricta:
1. Valores iniciales en 0 (bovinos=0, fraccion=0, personas=1, temp=20)
2. Placeholder actualizado: "Modifique los parámetros de la izquierda y haga clic en Calcular viabilidad."
3. Eliminar descripción de "Número de Bovinos"
4. Ajustar descripción de "Fracción de Recolección" a: "Ajuste el porcentaje de estiércol que puede recolectar diariamente."
5. Conservar la frase: "Ajuste según su capacidad de recolección diaria."
6. Eliminar el ejemplo del 25% y la nota del 25% de semiestabulado.
7. Clima detectado en resultados más visual, usando un badge con fondo de color según el clima.
8. Función resetCalculator() coherente.
9. Eliminación absoluta de comentarios.
"""
import re
...
def main():
    ...
    # 1. Modificar valores iniciales en el HTML
    # input-bovinos value="5" -> value="0"
    html_content = html_content.replace('id="input-bovinos" class="input-number" value="5"', 'id="input-bovinos" class="input-number" value="0"')
    
    # input-fraccion value="25" -> value="0"
    html_content = html_content.replace('id="input-fraccion" class="custom-slider" min="0" max="100" value="25"', 'id="input-fraccion" class="custom-slider" min="0" max="100" value="0"')
    # display-fraccion text 25% -> 0%
    html_content = html_content.replace('<span id="display-fraccion" class="slider-value-display">25%</span>', '<span id="display-fraccion" class="slider-value-display">0%</span>')
    # manure-calc-note text 10.0 -> 0.0
    html_content = html_content.replace('Estiércol recolectable: 10.0 kg/animal/día', 'Estiércol recolectable: 0.0 kg/animal/día')

    # input-personas value="4" -> value="1"
    html_content = html_content.replace('id="input-personas" class="custom-slider" min="1" max="10" value="4"', 'id="input-personas" class="custom-slider" min="1" max="10" value="1"')
    # display-personas text 4 personas -> 1 persona
    html_content = html_content.replace('<span id="display-personas" class="slider-value-display">4 personas</span>', '<span id="display-personas" class="slider-value-display">1 persona</span>')

    # input-temp value="16" -> value="20"
    html_content = html_content.replace('id="input-temp" class="custom-slider" min="-5" max="40" value="16"', 'id="input-temp" class="custom-slider" min="-5" max="40" value="20"')
    # display-temp text 16.0 °C -> 20.0 °C
    html_content = html_content.replace('<span id="display-temp" class="slider-value-display">16.0 °C</span>', '<span id="display-temp" class="slider-value-display">20.0 °C</span>')

    # 2. Modificar el texto del placeholder
    old_placeholder = '<p class="placeholder-text">Modifique los parámetros de la izquierda y haga clic en <strong>Calcular viabilidad</strong> para ver el análisis de prefactibilidad técnica y económica.</p>'
    new_placeholder = '<p class="placeholder-text">Modifique los parámetros de la izquierda y haga clic en <strong>Calcular viabilidad</strong>.</p>'
    html_content = html_content.replace(old_placeholder, new_placeholder)

    # 3. Eliminar descripción de "Número de Bovinos"
    old_bovinos_group = """                <div class="input-group">
    ...
    new_bovinos_group = """                <div class="input-group">
                    <div class="input-label">
                        <span>Número de Bovinos</span>
                    </div>
                    <div class="number-control">"""
    html_content = html_content.replace(old_bovinos_group, new_bovinos_group)

    # 4. Modificar descripción de "Fracción de Recolección" y nota aclaratoria
    # Buscamos la sección de Fracción de Recolección completa
    old_fraccion_block = """                <div class="input-group slider-container">
                    <div class="input-label">
                        <span>Fracción de Recolección</span>
                        <span id="display-fraccion" class="slider-value-display">0%</span>
                    </div>
                    <p class="input-description">Ajuste el porcentaje de estiércol que logra recolectar diariamente según su sistema de manejo y frecuencia (ej. 25% en semiestabulado con dos recolecciones al día).</p>
                    <input type="range" id="input-fraccion" class="custom-slider" min="0" max="100" value="0" step="1" oninput="updateFraccionDisplay()">
                    <p id="manure-calc-note" class="input-description" style="font-weight: 700; color: var(--primary-light); margin-top: 4px;">
                        Estiércol recolectable: 0.0 kg/animal/día
                    </p>
                    <p class="input-description" style="font-size: 0.8rem; font-style: italic; color: var(--accent-terracota); margin-top: 2px;">
                        * El valor del 25% corresponds al sistema semiestabulado (referencia del artículo). Ajuste según su capacidad de recolección diaria.
                    </p>
                </div>"""

    new_fraccion_block = """                <div class="input-group slider-container">
                    <div class="input-label">
                        <span>Fracción de Recolección</span>
                        <span id="display-fraccion" class="slider-value-display">0%</span>
                    </div>
                    <p class="input-description">Ajuste el porcentaje de estiércol que puede recolectar diariamente.</p>
                    <input type="range" id="input-fraccion" class="custom-slider" min="0" max="100" value="0" step="1" oninput="updateFraccionDisplay()">
                    <p id="manure-calc-note" class="input-description" style="font-weight: 700; color: var(--primary-light); margin-top: 4px;">
                        Estiércol recolectable: 0.0 kg/animal/día
                    </p>
                    <p class="input-description" style="font-size: 0.8rem; font-style: italic; color: var(--accent-terracota); margin-top: 2px;">
                        Ajuste según su capacidad de recolección diaria.
                    </p>
                </div>"""
    html_content = html_content.replace(old_fraccion_block, new_fraccion_block)

    # 5. Clima detectado en resultados más visual, usando un badge con fondo de color según el clima.
    # En el HTML de resultados:
    # <div id="res-clima" class="stat-value" style="font-size: 1.15rem; color: var(--primary-green); font-weight: 700;">Frío</div>
    # Lo reemplazamos por un contenedor que pueda tener clases de badge dinámicas
    old_clima_card = """                        <div class="stat-card">
                            <div class="stat-label">Clima Detectado</div>
                            <div id="res-clima" class="stat-value" style="font-size: 1.15rem; color: var(--primary-green); font-weight: 700;">Frío</div>
                        </div>"""
    
    new_clima_card = """                        <div class="stat-card">
                            <div class="stat-label">Clima Detectado</div>
    ...
    html_content = html_content.replace(old_clima_card, new_clima_card)

    # 6. Reemplazar el JavaScript por completo con la versión final que incluye:
    # - Valores iniciales correctos en reset (0, 0, 1, 20)
    # - Actualización del badge de clima en resultados dinámicamente
    # - Sin comentarios
    js_block = """<script>
    ...
    # Reemplazamos la etiqueta script por completo
    html_content = re.sub(r'<script>.*?</script>', js_block, html_content, flags=re.DOTALL)

    # 7. Eliminamos todos los comentarios HTML, CSS y JS de forma rigurosa
    html_content = remove_html_comments(html_content)
    
    style_matches = list(re.finditer(r'<style>(.*?)</style>', html_content, flags=re.DOTALL))
    for match in reversed(style_matches):
        style_content = match.group(0)
        cleaned_style = remove_css_comments(style_content)
        html_content = html_content[:match.start()] + cleaned_style + html_content[match.end():]
        
    script_matches = list(re.finditer(r'<script>(.*?)</script>', html_content, flags=re.DOTALL))
    for match in reversed(script_matches):
        script_content = match.group(0)
        cleaned_script = remove_js_comments(script_content)
        html_content = html_content[:match.start()] + cleaned_script + html_content[match.end():]
