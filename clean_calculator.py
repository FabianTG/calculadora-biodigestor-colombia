...
def main():
    ...
    # 1. Eliminar .custom-select del CSS
    css_pattern = r'\.custom-select\s*\{.*?\}(?:\s*\.custom-select:focus\s*\{.*?\})?'
    content = re.sub(css_pattern, '', content, flags=re.DOTALL)

    # 2. Reemplazar la sección de entradas para eliminar el selector y actualizar el slider de fracción
    inputs_pattern = r'<div class="input-group">\s*<div class="input-label">\s*<span>Número de Bovinos</span>\s*</div>.*?<!-- Entrada 2: Personas -->'
    ...
    content = re.sub(inputs_pattern, new_inputs, content, flags=re.DOTALL)

    # 3. Reemplazar la sección de JavaScript por completo
    ...
    # 4. Eliminar comentarios HTML, CSS y JS de forma rigurosa
    ...
    with open(path, 'w', encoding='utf-8') as f:
        f.write(content)
    print("Calculadora limpiada de forma absoluta.")
