...
# 1. Modificar la función getClimateParams(temp) para manejar la congelación por debajo de 10°C
new_get_climate_params = """        function getClimateParams(temp) {
            if (temp < 10) {
...
html = re.sub(
    r"function getClimateParams\(temp\)\s*\{.*?\}",
    new_get_climate_params,
    html,
    flags=re.DOTALL
)

# 2. Modificar la lógica de runCalculations() para manejar la inhibición y actualizar el badge de clima dinámicamente
# Busquemos la sección de clima y biogás en runCalculations()
# Vamos a reescribir runCalculations() para asegurar consistencia absoluta en el cálculo y visualización
new_run_calculations = """        function runCalculations() {
...
html = re.sub(
    r"function runCalculations\(\)\s*\{.*?\}\s*(?=function resetCalculator|\n\s*function resetCalculator)",
    new_run_calculations,
    html,
    flags=re.DOTALL
)

# 3. Modificar updateTempDisplay() para manejar la congelación por debajo de 10°C en el slider de la izquierda
new_update_temp_display = """        function updateTempDisplay() {
            const temp = parseFloat(document.getElementById('input-temp').value);
...
html = re.sub(
    r"function updateTempDisplay\(\)\s*\{.*?\}",
    new_update_temp_display,
    html,
    flags=re.DOTALL
)

# 4. Modificar el HTML del bloque de ahorro económico para tener el diseño de 3 columnas (Gasto Mensual, Ahorro Mensual, Ahorro a 5 Años)
# Busquemos el bloque <div id="res-savings-card" ...>
old_savings_card_regex = r'<div id="res-savings-card" class="savings-block".*?</div>\s*(?=<div id="res-recommendation"|<div id="res-alert-insufficient")'

new_savings_card = """<div id="res-savings-card" class="savings-block" style="background: linear-gradient(135deg, #FAF9F6, #F4F6F7); border: 1px solid var(--border-color); padding: 1.5rem; border-radius: 12px; margin-top: 1rem;">
...
html = re.sub(
    old_savings_card_regex,
    new_savings_card,
    html,
    flags=re.DOTALL
)

# 5. Modificar resetCalculator() para restablecer los nuevos campos correctamente
new_reset_calculator = """        function resetCalculator() {
...
html = re.sub(
    r"function resetCalculator\(\)\s*\{.*?\}",
    new_reset_calculator,
    html,
    flags=re.DOTALL
)

# 6. Modificar window.onload para inicializar la temperatura en 0
new_window_onload = """        window.onload = function() {
            document.getElementById('input-bovinos').value = 0;
            document.getElementById('input-fraccion').value = 0;
            document.getElementById('input-personas').value = 1;
            document.getElementById('input-temp').value = 0;
...
print("¡Ajustes de congelamiento por frío extremo y rediseño de dinero aplicados con éxito!")
