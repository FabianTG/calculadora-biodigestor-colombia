#!/usr/bin/env python3
"""
Reconstruye el archivo calculadora_biodigestor.html de forma robusta,
garantizando la presencia de todas las entradas (bovinos, fracción, personas, temperatura),
el botón 'Limpiar todo' al lado de 'Calcular', y la remoción absoluta de comentarios.
"""
import re
...
def main():
    ...
    # 1. Leemos el archivo actual para conservar los estilos y la estructura general
    with open(path, 'r', encoding='utf-8') as f:
        html_content = f.read()

    # 2. Reconstruimos la sección de inputs de la tarjeta card-inputs de forma limpia y directa
    # Buscamos desde <div class="card card-inputs"> hasta el inicio de <div class="card card-results">
    inputs_block = """            <div class="card card-inputs">
                <h2 class="card-title">
                ...
                <div class="input-group slider-container">
                    <div class="input-label">
                        ...
                        <span id="display-personas" class="slider-value-display">4 personas</span>
                    </div>
                    <p class="input-description">Determina la demanda energética útil para cocción basada en el consumo estándar de 0.166 kg GLP/persona/día.</p>
                    <input type="range" id="input-personas" class="custom-slider" min="1" max="10" value="4" step="1" oninput="updatePersonasDisplay()">
                </div>

                <div class="input-group slider-container">
                    <div class="input-label">
                        <span>Temperatura Media Anual</span>
                        <span id="display-temp" class="slider-value-display">16.0 °C</span>
                    </div>
                    <p class="input-description">La temperatura ambiente regula la velocidad de digestión bacteriana psicrófila/mesófila en sistemas tubulares de bajo costo.</p>
                    <input type="range" id="input-temp" class="custom-slider" min="-5" max="40" value="16" step="0.5" oninput="updateTempDisplay()">
                    <div id="badge-clima" class="climate-badge badge-cold">
                        ❄️ Clima Frío (&lt;18°C) – factor 0.55
                    </div>
                </div>
    ...
    # Reemplazamos la sección de inputs en el HTML
    pattern = r'<div class="card card-inputs">.*?</div>\s*<div class="card card-results">'
    html_content = re.sub(pattern, inputs_block + "\n\n            <div class=" + '"' + "card card-results" + '"' + ">", html_content, flags=re.DOTALL)

    # 3. Nos aseguramos de que el CSS de .btn-reset y .button-group-row esté en el style
    btn_reset_css = """
        .btn-reset {
    ...
    # Insertamos el CSS antes de </style> si no está ya
    if ".btn-reset" not in html_content:
        html_content = html_content.replace("</style>", btn_reset_css + "\n</style>")

    # 4. Reemplazamos el JavaScript por completo con la versión limpia sin residuos de manejo y con resetCalculator()
    js_block = """<script>
        ...
        function resetCalculator() {
            document.getElementById('input-bovinos').value = 5;
            document.getElementById('input-fraccion').value = 25;
            document.getElementById('input-personas').value = 4;
            document.getElementById('input-temp').value = 16;
    ...
    # 5. Eliminamos todos los comentarios HTML, CSS y JS de forma rigurosa
    ...
    # 6. Guardamos el archivo final reconstruido
