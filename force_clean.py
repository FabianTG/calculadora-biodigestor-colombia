#!/usr/bin/env python3
"""
Script para forzar la remoción del selector 'Sistema de Manejo del Ganado',
agregar el botón 'Limpiar todo' y limpiar absolutamente todos los comentarios.
"""
import re
...
def main():
    ...
    # 1. Remover .custom-select del CSS si existe
    content = re.sub(r'\.custom-select\s*\{.*?\}(?:\s*\.custom-select:focus\s*\{.*?\})?', '', content, flags=re.DOTALL)

    # 2. Agregar estilos para el botón "Limpiar todo" (.btn-reset)
    btn_reset_css = """
        .btn-reset {
    ...
    # Insertar el CSS del botón de reset antes del cierre de </style>
    content = content.replace("</style>", btn_reset_css + "\n</style>")

    # 3. Remover el bloque HTML del selector de manejo por completo
    # El bloque exacto que contiene select-manejo
    content = re.sub(r'<div class="input-group">\s*<div class="input-label">\s*<span>Sistema de Manejo del Ganado</span>.*?</div>\s*</div>', '', content, flags=re.DOTALL)
    # Por si acaso el regex anterior es muy específico, buscar una forma más genérica de eliminar el div que contiene select-manejo
    content = re.sub(r'<div class="input-group">[^<]*<div class="input-label">[^<]*<span>Sistema de Manejo del Ganado</span>.*?</div>\s*</select>\s*</div>', '', content, flags=re.DOTALL)

    # 4. Ajustar la descripción del input de bovinos para eliminar la mención al sistema de manejo
    content = content.replace(
        'Bovinos en pastoreo o confinamiento (el estiércol recolectable se calcula según el sistema de manejo seleccionado).',
        'Bovinos en pastoreo o confinamiento (el estiércol recolectable se calcula según la fracción de recolección seleccionada).'
    )

    # 5. Ajustar la descripción del slider de fracción de recolección
    slider_old_desc = 'Ajuste la fracción de estiércol que puede recolectar diariamente según su sistema de manejo y frecuencia de recolección (ej. 25% en semiestabulado con dos recolecciones al día).'
    slider_new_desc = 'Ajuste el porcentaje de estiércol que logra recolectar diariamente según su sistema de manejo y frecuencia (ej. 25% en semiestabulado con dos recolecciones al día).'
    content = content.replace(slider_old_desc, slider_new_desc)

    # 6. Eliminar el span custom-fraccion-note si existe
    content = re.sub(r'<span id="custom-fraccion-note".*?</span>', '', content, flags=re.DOTALL)

    # 7. Agregar el botón "Limpiar todo" en el HTML al lado del botón calcular usando la clase button-group-row
    old_btn_html = r'<button type="button" class="btn-calculate" onclick="triggerCalculation\(\)">\s*<span>📊 Calcular Viabilidad</span>\s*</button>'
    new_btn_html = """<div class="button-group-row">
    ...
    content = re.sub(old_btn_html, new_btn_html, content)

    # 8. Reemplazar la sección de JavaScript por completo con la versión limpia sin residuos de manejo y con resetCalculator()
    js_block = """<script>
    ...
    # Reemplazar la etiqueta script
    content = re.sub(r'<script>.*?</script>', js_block, content, flags=re.DOTALL)

    # 9. Eliminar comentarios HTML, CSS y JS de forma rigurosa
    ...
    with open(path, 'w', encoding='utf-8') as f:
        f.write(content)
    print("Calculadora procesada, selector removido, reset agregado y comentarios purgados.")
